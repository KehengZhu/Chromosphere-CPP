...
That is **107 test cases and 11643 individual checks, 0 failures**. Note that `Passed:` counts individual `EXPECT_NEAR` / `EXPECT_REL` / `EXPECT_TRUE` invocations across all cases, not the number of cases. The binary exits 0 if and only if no check failed. `ctest` currently reports `100% tests passed, 0 tests failed out of 9`.
...
**The lower chromosphere cannot accumulate mass in float32, and its mass flux is therefore not a steady-state statement.** `mixture_advance` computes `next = state + dt*rhs` on a float32 state. In the lower chromosphere `dt*(-df/ds)` is about `2.6e-17 kg m^-3 s^-1` while one float32 ULP of `rho ~ 9e-10` is about `5.6e-17`, so **619 of 661 cells sit below half a ULP** and the addition rounds straight back to `rho`. There is no compensated summation, so this is permanent stagnation, not slow drift. The visible consequence is that the conservative face mass flux of the 4000 s release run is 2.8x larger at the 1600 km base than at the top, and never flattens, although continuity forces a quasi-steady constant-area column to carry a height-independent flux. A matched double-precision control — identical physics, grid, boundary conditions, reconstruction, flux, CFL and step count, only `chromosphere::Vec` promoted to `double` — brings the base/top ratio to 0.996 and the 1600–1850 km spread from 119 % to 2 %, and closes the column mass budget to 6 % where float32 fails it by a factor of 200 *with the wrong sign*. **So: do not read a lower-chromosphere flux gradient as physics, and do not chase it as a boundary-condition bug.** Any quantitative chromospheric mass-budget or long-time mass-loading claim needs a double-precision state or a perturbation-variable formulation first. Top-of-domain evaporation observables shift by only about 3 %. Evidence: `docs/studies/numerics/float32_precision_control_experiment.md`.

**A localized first-interior-face artifact is real and open.** Distinct from, and much narrower than, the float32 gradient above: face 0 carries about 1900x the interior median `|f_diff|`, its `|f_diff|` is 16 % of the local `f_total`, and the cell-0 `rho V` overshoots `f_total[0]` by 28 %. The double-precision control leaves it unchanged (16.3 % versus 15.9 %), so it is a genuine scheme or inner-ghost-closure effect rather than round-off, and it has not been diagnosed.
...
**The `.faceflux` divergence anomaly is explained, and it was not a capture bug.** The sidecar's flux *values* were always consistent; its *divergence* disagreed with the density evolution recorded in the same file by factors of order 60x in the lower chromosphere, which is exactly the float32 mass-update stagnation described above — the fluxes were right and the density *response* was forbidden. In the double-precision control the same integrated divergence matches the recorded density change to 6 %. `R_total` and `R_eff` in `util/face_flux_diag.py` are therefore usable as flux diagnostics, but any mass-budget residual built from them in a float32 run measures the representation floor, not the scheme. Separately and still unexplained: a normalized-curvature floor of about `1.4e-4` in `Q_eff` does not decrease with refinement; it is harmless at that magnitude.
...
| `docs/studies/validation/long_run_output_and_cfl_validation.md` | Sweep configuration retired; conclusions current | The CFL 0.50 acceptance sweep, the `CHROMO_T_END` / `CHROMO_FRAME_DT` / `CHROMO_STEP_CAP` run-control semantics, and the float32 velocity noise-floor finding. |
...
| `docs/studies/numerics/float32_precision_control_experiment.md` | **Current** — the authority for the storage-precision limitation | That the broad lower-chromosphere mass-flux gradient is float32 conserved-state round-off and not physics, the below-half-a-ULP mass-update mechanism, the closure of the `.faceflux` divergence anomaly, the precision-independence of the first-interior-face artifact, and the `CHROMO_STATE_FLOAT64` diagnostic build. |
| `docs/studies/numerics/roe_n1000_lower_chromosphere_sloshing_diagnosis.md` | Current for the mode; its `.faceflux` divergence anomaly is now explained as float32 round-off; measured under the Roe reference flux | The lower-chromosphere standing mode, the reflecting inner boundary, and the mass-reservoir behaviour. |
