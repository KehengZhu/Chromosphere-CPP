...
# ISO_RECONSTRUCTION overrides — Roe + (ln rho,V,ln p) are the defaults):
#
#   1. default-path equivalence — the no-override run must reproduce an explicit
#      ISO_RIEMANN=roe-local ISO_RECONSTRUCTION=lnrho-v-lnp run bitwise (apart
...
ISO_RIEMANN=roe-local ISO_RECONSTRUCTION=lnrho-v-lnp \
...
SMOKE="${OUT_DIR}/release_roe_lnp_N500_100s.txt"
