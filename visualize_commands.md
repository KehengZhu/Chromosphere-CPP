...
- All plotting/animation scripts live in `util/` (two physics-reference scripts live in `visualization/` itself and write next to themselves). Outputs land in `visualization/` (gitignored).
- Input simulation dumps live under `outputs/`; loop/field-line geometry `.dat` files live under `scenarios/data/`; magnetograms under `util/data/`. Some `outputs/` subdirs are gitignored — adjust the input paths below to your actual run dumps where noted.
...
build/chromo_main outputs/model_c7_movie.txt full ionization model_c7 "" 1.5
.venv/bin/python util/animate_output.py outputs/model_c7_movie.txt visualization/model_c7_evolution.mp4 45
```
Positional: `[input.txt] [out.mp4] [fps]` — defaults `build/output.txt`, `visualization/model_c7_evolution.mp4`, `15`.

### `model_c7_explicit_evolution.mp4`
Same 9-panel animation, for the explicit-scheme (R_I = 0) run.
```bash
.venv/bin/python util/animate_output.py outputs/output_explicit.txt visualization/model_c7_explicit_evolution.mp4
```

### `model_c7_snapshot.png`
Single 8-panel snapshot of the last frame of a run.
```bash
.venv/bin/python util/plot_output.py outputs/model_c7_movie.txt visualization/model_c7_snapshot.png
```
Positional: `[input.txt] [out.png]` — defaults `build/output.txt`, `visualization/model_c7_snapshot.png`.

### `model_c7_compare.mp4`
8-panel animation overlaying two runs: semi-implicit (full) vs explicit (R_I = 0).
```bash
.venv/bin/python util/animate_compare.py outputs/output_full.txt outputs/output_explicit.txt visualization/model_c7_compare.mp4
```
Positional: `[full.txt] [explicit.txt] [out.mp4] [fps]` — defaults `build/output_full.txt`, `build/output_explicit.txt`, `visualization/model_c7_compare.mp4`, `15`.
...
```bash
.venv/bin/python util/animate_cooling_compare.py outputs/out_c7_cool_on.txt outputs/out_c7_cool_off.txt visualization/model_c7_cooling_compare.mp4
```
Positional: `[cool_on.txt] [cool_off.txt] [out.mp4] [fps]` — defaults `outputs/out_c7_cool_on.txt`, `outputs/out_c7_cool_off.txt`, `visualization/model_c7_cooling_compare.mp4`, `6`.

### `model_c7_ionization_compare.mp4`
8-panel animation overlaying ionization ON (Stage E) vs OFF.
```bash
.venv/bin/python util/animate_ionization_compare.py outputs/out_ioniz_on.txt outputs/out_ioniz_off.txt visualization/model_c7_ionization_compare.mp4
```
...
```bash
.venv/bin/python util/visualize_rho_total.py outputs/out_c7_openbc.txt --png visualization/c7_openbc_rho_total.png --mp4 visualization/c7_openbc_rho_total.mp4
```
...
```bash
.venv/bin/python util/visualize_rho_total.py outputs/out_c7_10x_on.txt
```
(default output)
...
```bash
.venv/bin/python util/visualize_rho_total.py outputs/out_pfss_10x.txt
```
(default output)
...
```bash
.venv/bin/python util/visualize_rho_total_compare.py outputs/out_pfss_10x.txt outputs/out_pfss_10x_noioniz.txt
```
...
```bash
.venv/bin/python util/visualize_rho_total_compare.py outputs/out_c7_openbc.txt outputs/out_c7_openbc_noioniz.txt --out-png visualization/c7_openbc_ioniz_compare.png --out-mp4 visualization/c7_openbc_ioniz_compare.mp4
```

### `pfss_openbc_ioniz_compare.png`, `pfss_openbc_ioniz_compare.mp4`
ρ_tot ionization on/off comparison for the open-BC PFSS case.
```bash
.venv/bin/python util/visualize_rho_total_compare.py outputs/out_pfss_openbc.txt outputs/out_pfss_openbc_noioniz.txt --out-png visualization/pfss_openbc_ioniz_compare.png --out-mp4 visualization/pfss_openbc_ioniz_compare.mp4
```
...
```bash
.venv/bin/python util/animate_compare.py outputs/out_c7_openbc.txt outputs/out_c7_oldlike_openbc.txt visualization/c7_openbc_8panel_compare.mp4
.venv/bin/python util/animate_compare.py outputs/out_pfss_openbc.txt outputs/out_pfss_openbc_noioniz.txt visualization/pfss_openbc_8panel_compare.mp4
```
...
```bash
.venv/bin/python util/visualize_flare.py outputs/flare_explosive.txt outputs/flare_gentle.txt 25.0
```
Positional (defaults): `explosive.txt`=`outputs/flare_explosive.txt`, `gentle.txt`=`outputs/flare_gentle.txt`, `t_max_s`=`25.0`.
...
```bash
.venv/bin/python util/visualize_flare_frames.py outputs/flare_explosive_500.txt outputs/flare_gentle.txt
```
...
FLARE_DELTA=5 FLARE_E_CUT=20 FLARE_T_ON=2 FLARE_DUR=10 \
  .venv/bin/python util/animate_loop_flare.py scenarios/data/loop_full_fine.dat outputs/loop_full_flare.txt visualization/loop_full_flare_evolution.mp4
# Half (closed) loop
.venv/bin/python util/animate_loop_flare.py scenarios/data/loop_closed_fine.dat outputs/loop_half_flare.txt visualization/loop_flare_evolution.mp4
```
Positional: `[loop.dat] [flare.txt] [out.mp4] [t_max_s]` — defaults `scenarios/data/loop_full_fine.dat`, `outputs/loop_full_flare.txt`, `visualization/loop_full_flare_evolution.mp4`.
...
```bash
.venv/bin/python util/verify_loop_flare.py scenarios/data/loop_closed_fine.dat outputs/loop_closed_quiet.txt outputs/loop_closed_flare.txt
```
Positional (defaults): `<loop.dat>`=`scenarios/data/loop_closed_test.dat`, `<control.txt>`=`outputs/loop_closed_quiet.txt`, `<flare.txt>`=`outputs/loop_closed_flare.txt`.

### `loop_cooling_full.png`, `loop_cooling_half.png`
Post-beam cooldown analysis (apex/EM-weighted coronal T & n vs time, e-folding cooling times, apex (n,T) trajectory). Default output `visualization/loop_cooling.png`; pass an explicit name for the full/half variants.
```bash
.venv/bin/python util/loop_cooling.py scenarios/data/loop_full_fine.dat   outputs/loop_full_flare.txt visualization/loop_cooling_full.png
.venv/bin/python util/loop_cooling.py scenarios/data/loop_closed_fine.dat outputs/loop_half_flare.txt visualization/loop_cooling_half.png
```
...
```bash
.venv/bin/python util/check_loop_ic.py scenarios/data/loop_full_fine.dat visualization/loop_ic_check.png
```
Positional (defaults): `<loop.dat>`=`scenarios/data/loop_closed_test.dat`, `[out.png]`=`visualization/loop_ic_check.png`.
...
```bash
.venv/bin/python util/beam_stopping_feedback.py scenarios/data/loop_full_fine.dat outputs/loop_full_thick.txt visualization/beam_stopping_feedback.png 20 5
```
...
.venv/bin/python util/visualize_pfss_lines_evolution.py \
    ...
    --outs outputs/out_lineA.txt outputs/out_lineB.txt outputs/out_lineC.txt \
    --out-png visualization/pfss_lines_evolution.png \
    --out-mp4 visualization/pfss_lines_evolution.mp4
```
...
```bash
.venv/bin/python util/animate_pfss_flare.py outputs/pfss_lineA_flare.txt visualization/pfss_lineA_flare_evolution.mp4
```
Positional: `[input.txt] [out.mp4]` — defaults `outputs/pfss_lineA_flare.txt`, `visualization/pfss_lineA_flare_evolution.mp4`.

### `pfss_10x_evolution.mp4`
Same 4-panel flare-profile animation applied to the 10×-duration PFSS relaxation run.
```bash
.venv/bin/python util/animate_pfss_flare.py outputs/out_pfss_10x.txt visualization/pfss_10x_evolution.mp4
```
...
```bash
.venv/bin/python util/verify_pfss_flare.py outputs/pfss_lineA_control.txt outputs/pfss_lineA_flare.txt visualization/pfss_lineA_flare_verify.png
```
...
.venv/bin/python util/visualize_canopy.py \
    --sim-output outputs/out_canopy.txt \
    --out-png visualization/canopy_geometry_evolution.png \
    --out-mp4 visualization/canopy_evolution.mp4
```
...
.venv/bin/python util/visualize_event_3d.py \
    --lines-json outputs/event_ensemble_lines.json \
    --magnetogram util/data/adapt40311_044012_202408010600_i00053600n1.fts.gz \
    --out-dir visualization --no-anim
mv visualization/event_ensemble_3d_peak.png visualization/event_ensemble_3d_global.png
```
...
FLARE_DELTA=4 FLARE_E_CUT=20 FLARE_T_ON=2 FLARE_DUR=10 \
  .venv/bin/python util/animate_loop_flare.py scenarios/data/event_20240801_AR13768.dat outputs/event_20240801_AR13768_flare.txt visualization/event_20240801_AR13768_flare.mp4
FLARE_DELTA=4 FLARE_E_CUT=20 FLARE_T_ON=2 FLARE_DUR=10 \
  .venv/bin/python util/animate_loop_flare.py scenarios/data/event_20240801_AR13768.dat outputs/event_te/event_20240801_AR13768_te.txt visualization/event_20240801_AR13768_Te.mp4
```
...
```bash
FLARE_DELTA=4 FLARE_E_CUT=20 FLARE_T_ON=2 FLARE_DUR=10 \
  .venv/bin/python util/animate_loop_flare.py scenarios/data/event_ensemble/c05_101Mm.dat  outputs/event_te/c05_101Mm_te.txt  visualization/event_20240801_c05_101Mm_Te.mp4
FLARE_DELTA=4 FLARE_E_CUT=20 FLARE_T_ON=2 FLARE_DUR=10 \
  .venv/bin/python util/animate_loop_flare.py scenarios/data/event_ensemble/o00_1040Mm.dat outputs/event_te/o00_1040Mm_te.txt visualization/event_20240801_o00_1040Mm_Te.mp4
```
...
```bash
FLARE_DELTA=4 FLARE_E_CUT=20 FLARE_T_ON=2 FLARE_DUR=10 \
  .venv/bin/python util/animate_loop_flare.py scenarios/data/event_ensemble/c05_101Mm.dat  outputs/event_ensemble/c05_101Mm.txt  visualization/event_ensemble_c05_101Mm_closed.mp4
FLARE_DELTA=4 FLARE_E_CUT=20 FLARE_T_ON=2 FLARE_DUR=10 \
  .venv/bin/python util/animate_loop_flare.py scenarios/data/event_ensemble/o00_1040Mm.dat outputs/event_ensemble/o00_1040Mm.txt visualization/event_ensemble_o00_1040Mm_open.mp4
```
...
```bash
.venv/bin/python util/synth_doppler.py scenarios/data/event_ensemble/c05_101Mm.dat outputs/event_te/c05_101Mm_te.txt c05_101Mm_te
```

### `o00_1040Mm_te_{doppler,dopplergram}.png`
Synthetic IRIS Doppler (open 1040 Mm line, T_e≠T_i run).
```bash
.venv/bin/python util/synth_doppler.py scenarios/data/event_ensemble/o00_1040Mm.dat outputs/event_te/o00_1040Mm_te.txt o00_1040Mm_te
```

### `event_20240801_AR13768_te_{doppler,dopplergram}.png`
Synthetic IRIS Doppler (2024-08-01 AR13768 line, T_e≠T_i run).
```bash
.venv/bin/python util/synth_doppler.py scenarios/data/event_20240801_AR13768.dat outputs/event_20240801_AR13768_te.txt event_20240801_AR13768_te
```

### `sol20140329_10Mm_1T_{doppler,dopplergram}.png`
SOL2014-03-29 10 Mm loop, single-temperature (T_e=T_i) run.
```bash
.venv/bin/python util/synth_doppler.py scenarios/data/sol20140329_10Mm.dat outputs/sol2014/sol20140329_10Mm_1T.txt sol20140329_10Mm_1T
```

### `sol20140329_10Mm_te_{doppler,dopplergram}.png`
SOL2014-03-29 10 Mm loop, T_e≠T_i run.
```bash
.venv/bin/python util/synth_doppler.py scenarios/data/sol20140329_10Mm.dat outputs/sol2014/sol20140329_10Mm_te.txt sol20140329_10Mm_te
```

### `sol20140329_10Mm_te_stab_{doppler,dopplergram}.png`
SOL2014-03-29 10 Mm loop, T_e≠T_i, numerically-stabilized variant.
```bash
.venv/bin/python util/synth_doppler.py scenarios/data/sol20140329_10Mm.dat outputs/sol2014/sol20140329_10Mm_te_stab.txt sol20140329_10Mm_te_stab
```

### `sol20140329_dur6_te_{doppler,dopplergram}.png`
SOL2014-03-29 10 Mm loop, T_e≠T_i, 6 s beam-duration variant.
```bash
FLARE_DUR=6 .venv/bin/python util/synth_doppler.py scenarios/data/sol20140329_10Mm.dat outputs/sol2014/sol20140329_dur6_te.txt sol20140329_dur6_te
```
...
```bash
.venv/bin/python util/compare_doppler_obs.py scenarios/data/sol20140329_10Mm.dat outputs/sol2014/sol20140329_10Mm_te.txt sol20140329_10Mm_te
FLARE_DUR=6 .venv/bin/python util/compare_doppler_obs.py scenarios/data/sol20140329_10Mm.dat outputs/sol2014/sol20140329_dur6_te.txt sol20140329_dur6_te
```
...
```bash
.venv/bin/python util/diag_two_fluid.py scenarios/data/event_ensemble/c05_101Mm.dat  outputs/two_fluid_test/c05_closed.txt c05_closed
.venv/bin/python util/diag_two_fluid.py scenarios/data/event_ensemble/o00_1040Mm.dat outputs/two_fluid_test/o00_open.txt   o00_open
.venv/bin/python util/diag_two_fluid.py scenarios/data/event_ensemble/c05_101Mm.dat  outputs/two_fluid_test/starter.txt   starter
```
...
```bash
.venv/bin/python util/compare_two_fluid.py scenarios/data/event_ensemble/c05_101Mm.dat  outputs/two_fluid_test/c05_neutrals_on.txt outputs/two_fluid_test/c05_neutrals_off.txt c05_closed
.venv/bin/python util/compare_two_fluid.py scenarios/data/event_ensemble/o00_1040Mm.dat outputs/two_fluid_test/o00_neutrals_on.txt outputs/two_fluid_test/o00_neutrals_off.txt o00_open
```

### `two_fluid/{c05,o00}_neutrals_{on,off}.mp4`
Per-run movies of the neutrals-on / neutrals-off two-fluid runs (the same `outputs/two_fluid_test/` dumps used by the comparison PNGs above), animated with `animate_loop_flare.py` and written under `visualization/two_fluid/`.
```bash
.venv/bin/python util/animate_loop_flare.py scenarios/data/event_ensemble/c05_101Mm.dat  outputs/two_fluid_test/c05_neutrals_on.txt  visualization/two_fluid/c05_neutrals_on.mp4
.venv/bin/python util/animate_loop_flare.py scenarios/data/event_ensemble/c05_101Mm.dat  outputs/two_fluid_test/c05_neutrals_off.txt visualization/two_fluid/c05_neutrals_off.mp4
.venv/bin/python util/animate_loop_flare.py scenarios/data/event_ensemble/o00_1040Mm.dat outputs/two_fluid_test/o00_neutrals_on.txt  visualization/two_fluid/o00_neutrals_on.mp4
.venv/bin/python util/animate_loop_flare.py scenarios/data/event_ensemble/o00_1040Mm.dat outputs/two_fluid_test/o00_neutrals_off.txt visualization/two_fluid/o00_neutrals_off.mp4
```

---

## 6. Physics reference plots (scripts live in `visualization/`)
...
```bash
.venv/bin/python visualization/plot_c7_ioniz_recomb_rates.py
```
...
```bash
.venv/bin/python visualization/plot_heat_conductivity_c7.py
```
> Note: a correctly-spelled `heat-conductivities-c7-wide-range.png` also exists in `visualization/` but is **not** emitted by this script's default paths (the wide default is the misspelled `conductivies`). It is a renamed copy — `mv` the wide output if you want the corrected name.
...
```bash
.venv/bin/python util/extract_event_loop.py --magnetogram util/data/adapt40311_044012_202408010600_i00005600n1.fts.gz --ar-lon 74 --ar-lat -16 --obstime 2024-08-01T07:09:00 --out-json outputs/event_20240801_AR13768_geometry.json
```
...
```bash
# Mode A (manifest):
.venv/bin/python util/run_ensemble.py --manifest outputs/event_ensemble_jobs.json --workers 16
# Mode B (glob, shared env):
.venv/bin/python util/run_ensemble.py --dats "scenarios/data/event_ensemble/*.dat" --out-dir outputs/event_ensemble --env PFSS_FLARE=1 FLARE_BEAM_FLUX=3e7 FLARE_DELTA=4 FLARE_E_CUT=20
```
...
```bash
cp visualization/event_20240801_c05_101Mm_Te.mp4 visualization/event_20240801_o00_1040Mm_Te.mp4 docs/presentation-build/
```
...
`time_mult=2.0` (manifest `outputs/event_te_2x_jobs.json`); NB `time_mult` scales the *step cap* and dt grows
...
.venv/bin/python util/run_ensemble.py --manifest outputs/event_te_2x_jobs.json --workers 2
...
env ${=ENV} .venv/bin/python util/animate_loop_flare.py scenarios/data/event_ensemble/c05_101Mm.dat  outputs/event_te/c05_101Mm_te.txt  visualization/event_20240801_c05_101Mm_Te.mp4 100
env ${=ENV} .venv/bin/python util/animate_loop_flare.py scenarios/data/event_ensemble/o00_1040Mm.dat outputs/event_te/o00_1040Mm_te.txt visualization/event_20240801_o00_1040Mm_Te.mp4 100
# (2) extract the first-frame poster shown on each slide
ffmpeg -y -i visualization/event_20240801_c05_101Mm_Te.mp4  -frames:v 1 -q:v 2 visualization/poster_c05_closed.jpg
ffmpeg -y -i visualization/event_20240801_o00_1040Mm_Te.mp4 -frames:v 1 -q:v 2 visualization/poster_o00_open.jpg
```
...
GENTLE=1 ./build/chromo_main outputs/gentle_relax.txt full ionization \
    pfss_field_line scenarios/data/loop_closed_gentle.dat 12 cooling
# → visualization/gentle_relax_convergence.png : max|V|(t), mean|V|(t), max|dlnT/dt|, T(t)
...
GENTLE=1 GENTLE_ENHANCE=3 GENTLE_T_ON=1000 GENTLE_RAMP=200 ./build/chromo_main \
    outputs/gentle_evap.txt full ionization pfss_field_line \
    scenarios/data/loop_closed_gentle.dat 16 cooling
# → visualization/gentle_evap_signature.png : coronal EM rise, TR upflow, T step, upflow Mach (≪1)
python util/gentle_evaporation_diag.py outputs/gentle_evap.txt visualization/gentle_evap_signature.png 1000 200
# → visualization/gentle_evap_profiles.png : split-axis preflare-vs-evaporation T, n_e, V profiles
python util/gentle_evap_profiles.py scenarios/data/loop_closed_gentle.dat \
    outputs/gentle_evap.txt visualization/gentle_evap_profiles.png 1000
...
./build/chromo_main outputs/gentle_v2_relax.txt full ionization model_gentle - 6 cooling
python util/check_relaxation.py outputs/gentle_v2_relax.txt visualization/gentle_v2_relax_convergence.png

# Phase B+C combined — relax (to t=1000 s) then ramp q(T) ×3 over 120 s
GENTLE_Q_ENHANCE=3 GENTLE_Q_T_ON=1000 GENTLE_Q_RAMP=120 \
    ./build/chromo_main outputs/gentle_v2_evap.txt full ionization model_gentle - 8 cooling
...
python util/animate_gentle_column.py outputs/gentle_v2_evap.txt \
    visualization/gentle_v2_evolution.mp4 1000 120 3
```
...
GENTLE_NS=300 ./build/chromo_main outputs/gentle_v3_relax.txt full ionization model_gentle - 6 cooling
...
GENTLE_NS=300 GENTLE_Q_ENHANCE=3 GENTLE_Q_T_ON=3000 GENTLE_Q_RAMP=300 \
    ./build/chromo_main outputs/gentle_v3_evap.txt full ionization model_gentle - 12 cooling
...
python util/animate_gentle_column.py outputs/gentle_v3_evap.txt \
    visualization/gentle_v3_evolution.mp4 3000 300 3
...
GENTLE_NS=300 GENTLE_T_BOOST=1.5 GENTLE_Q_ENHANCE=1 \
    ./build/chromo_main outputs/gentle_hotcorona.txt full ionization model_gentle - 6 cooling
...
SLOW_T0=0 SLOW_T1=200 SLOW_FACTOR=4 .venv/bin/python util/animate_gentle_column.py \
    outputs/gentle_hotcorona.txt visualization/gentle_hotcorona_evolution.mp4 1e9 1 1
```
...
GENTLE_NS=300 GENTLE_HEAT=1 GENTLE_HEAT_SH_FRAC=3.0 GENTLE_HEAT_TMAX_MK=1.3 \
    ./build/chromo_main outputs/gentle_Hs_uniform.txt full ionization model_gentle - 10 cooling
SLOW_T0=0 SLOW_T1=300 SLOW_FACTOR=4 .venv/bin/python util/animate_gentle_column.py \
    outputs/gentle_Hs_uniform.txt visualization/gentle_Hs_evolution.mp4 1e9 1 1
```
...
```bash
ISO_HEAT_FLUX=0 ./build/chromo_main outputs/iso_stage1.txt full no-ionization model_isentropic - 20 no-cooling
...
```bash
ISO_HEAT_FLUX=1 ISO_TJUMP_A=10 ISO_TJUMP_B=5 ./build/chromo_main outputs/iso_stage2.txt full no-ionization model_isentropic - 20 no-cooling
...
```bash
ISO_HEAT_FLUX=1 ISO_COOLING=1 ISO_TJUMP_A=10 ISO_TJUMP_B=5 ./build/chromo_main outputs/iso_stage2b.txt full no-ionization model_isentropic - 20 no-cooling
...
```bash
.venv/bin/python util/animate_isentropic.py outputs/iso_stage1.txt
.venv/bin/python util/animate_isentropic.py outputs/iso_stage2.txt
.venv/bin/python util/animate_isentropic.py outputs/iso_stage2b.txt   # default input if omitted
```
...
ISO_C7_IC=1 ISO_RELAX_TIME=1500 ISO_INNER_T_NEUMANN=1 ISO_HEAT_FLUX=1 ISO_TJUMP_A=10 ISO_TJUMP_B=5 ./build/chromo_main outputs/iso_stage2_ext.txt full no-ionization model_isentropic - 60 no-cooling
.venv/bin/python util/animate_isentropic.py outputs/iso_stage2_ext.txt
```
...
ISO_C7_IC=1 ISO_C7_HSE=1 ISO_H_BASE=0 ISO_DH=2152.6 ISO_T_TOP=22000 ISO_HEAT_FLUX=1 ISO_LOG_RECON=1 ISO_INNER_WB=1 ISO_NS=4000 ./build/chromo_main outputs/iso_t22k.txt full no-ionization model_isentropic - 120 no-cooling   # stop ~t=2080s
.venv/bin/python util/animate_isentropic.py outputs/iso_t22k.txt
.venv/bin/python util/plot_iso_frame.py outputs/iso_t22k.txt   # final-frame snapshot
...
ISO_C7_IC=1 ISO_C7_HSE=1 ISO_H_BASE=0 ISO_DH=2152.6 ISO_T_TOP=22000 ISO_HEAT_FLUX=1 ISO_LOG_RECON=1 ISO_INNER_WB=1 ISO_NS=4000 ISO_GAMMA=1.05 ./build/chromo_main outputs/iso_t22k_gamma105.txt full no-ionization model_isentropic - 120 no-cooling   # stop ~t=2140s
ISO_GAMMA=1.05 .venv/bin/python util/animate_isentropic.py outputs/iso_t22k_gamma105.txt visualization/iso_t22k_evolution_gamma105.mp4   # -> iso_t22k_evolution_gamma105.mp4

# ns=2000 companion (same config, half resolution; for the grid-convergence comparison). Auto-stopped ~t=2208s.
ISO_C7_IC=1 ISO_C7_HSE=1 ISO_H_BASE=0 ISO_DH=2152.6 ISO_T_TOP=22000 ISO_HEAT_FLUX=1 ISO_LOG_RECON=1 ISO_INNER_WB=1 ISO_NS=2000 ./build/chromo_main outputs/iso_t22k_ns2000.txt full no-ionization model_isentropic - 120 no-cooling   # stop ~t=2080s (overshot to 2208s)
.venv/bin/python util/animate_isentropic.py outputs/iso_t22k_ns2000.txt visualization/iso_t22k_evolution_ns2000.mp4   # -> iso_t22k_evolution_ns2000.mp4
...
ISO_C7_IC=1 ISO_C7_HSE=1 ISO_H_BASE=0 ISO_DH=2152.6 ISO_T_TOP=22000 ISO_HEAT_FLUX=1 ISO_LOG_RECON=1 ISO_INNER_WB=1 ISO_NS=2000 ISO_GAMMA=1.05 ./build/chromo_main outputs/iso_t22k_ns2000_gamma105.txt full no-ionization model_isentropic - 120 no-cooling   # stop ~t=2300s
ISO_GAMMA=1.05 .venv/bin/python util/animate_isentropic.py outputs/iso_t22k_ns2000_gamma105.txt visualization/iso_t22k_evolution_ns2000_gamma105.mp4   # -> iso_t22k_evolution_ns2000_gamma105.mp4
```
...
ISO_C7_IC=1 ISO_C7_HSE=1 ISO_H_BASE=0 ISO_DH=2152.6 ISO_T_TOP=22000 ISO_HEAT_FLUX=1 ISO_LOG_RECON=1 ISO_INNER_WB=1 ISO_NS=2000 ISO_GAMMA=1.05 ISO_MC3=1 ISO_MC3_BETA=2 ./build/chromo_main outputs/iso_t22k_ns2000_gamma105_mc3.txt full no-ionization model_isentropic - 120 no-cooling   # stop ~t=2300s
ISO_GAMMA=1.05 .venv/bin/python util/animate_isentropic.py outputs/iso_t22k_ns2000_gamma105_mc3.txt visualization/iso_t22k_evolution_ns2000_gamma105_mc3.mp4

# fix (inner-BC density WB) + MC3 — best config
ISO_C7_IC=1 ISO_C7_HSE=1 ISO_H_BASE=0 ISO_DH=2152.6 ISO_T_TOP=22000 ISO_HEAT_FLUX=1 ISO_LOG_RECON=1 ISO_INNER_WB=1 ISO_INNER_WB_RHO=1 ISO_MC3=1 ISO_MC3_BETA=2 ISO_NS=2000 ISO_GAMMA=1.05 ./build/chromo_main outputs/iso_t22k_ns2000_gamma105_mc3_wbrho.txt full no-ionization model_isentropic - 120 no-cooling   # stop ~t=2300s
ISO_GAMMA=1.05 .venv/bin/python util/animate_isentropic.py outputs/iso_t22k_ns2000_gamma105_mc3_wbrho.txt visualization/iso_t22k_evolution_ns2000_gamma105_mc3_wbrho.mp4
```
...
ISO_C7_IC=1 ISO_C7_HSE=1 ISO_H_BASE=0 ISO_DH=2152.6 ISO_T_TOP=22000 ISO_HEAT_FLUX=1 ISO_LOG_RECON=1 ISO_INNER_WB=1 ISO_INNER_WB_RHO=1 ISO_EQ_WB=1 ISO_MC3=1 ISO_MC3_BETA=2 ISO_NS=2000 ISO_GAMMA=1.05 ./build/chromo_main outputs/iso_t22k_ns2000_gamma105_bestwb_condon.txt full no-ionization model_isentropic - 120 no-cooling   # stop ~t=2300s
ISO_GAMMA=1.05 .venv/bin/python util/animate_isentropic.py outputs/iso_t22k_ns2000_gamma105_bestwb_condon.txt visualization/iso_t22k_evolution_ns2000_gamma105_bestwb.mp4

# same config, conduction OFF (ISO_HEAT_FLUX=0) — the equilibrium reference for the comparison
ISO_C7_IC=1 ISO_C7_HSE=1 ISO_H_BASE=0 ISO_DH=2152.6 ISO_T_TOP=22000 ISO_HEAT_FLUX=0 ISO_LOG_RECON=1 ISO_INNER_WB=1 ISO_INNER_WB_RHO=1 ISO_EQ_WB=1 ISO_MC3=1 ISO_MC3_BETA=2 ISO_NS=2000 ISO_GAMMA=1.05 ./build/chromo_main outputs/iso_t22k_ns2000_gamma105_bestwb_condoff.txt full no-ionization model_isentropic - 120 no-cooling   # stop ~t=2300s

# mass-flux comparison movie (item 2): conduction ON vs OFF overlaid (defaults to the two files above)
.venv/bin/python util/animate_iso_condcompare.py   # -> visualization/iso_t22k_ns2000_gamma105_bestwb_condcompare.mp4
```
...
```bash
ISO_C7_IC=1 ISO_C7_HSE=1 ISO_H_BASE=0 ISO_DH=2152.6 ISO_T_TOP=22000 ISO_HEAT_FLUX=1 ISO_LOG_RECON=1 ISO_INNER_WB=1 ISO_INNER_WB_RHO=1 ISO_EQ_WB=1 ISO_MC3=1 ISO_MC3_BETA=2 ISO_NS=2000 ISO_GAMMA=1.05 ISO_TRAC=1 ISO_TRAC_TCHROM=8000 ISO_TRAC_TC_FIXED=22000 ./build/chromo_main outputs/iso_t22k_ns2000_gamma105_tracon.txt full no-ionization model_isentropic - 4 no-cooling
ISO_GAMMA=1.05 .venv/bin/python util/animate_isentropic.py outputs/iso_t22k_ns2000_gamma105_tracon.txt visualization/iso_t22k_evolution_ns2000_gamma105_tracon.mp4
```
...
```bash
.venv/bin/python util/plot_cond_hydro_diagnostic.py --time 100 --out visualization/iso_cond_hydro_convergence_t100.png \
  outputs/iso_cond_hydro_ns2000.csv outputs/iso_cond_hydro_ns3000.csv outputs/iso_cond_hydro_ns4000.csv
```
...
.venv/bin/python util/plot_cond_hydro_diagnostic.py --time 100 \
  ...
  outputs/iso_cond_hydro_local4_outer2000.csv outputs/iso_cond_hydro_local4_outer3000.csv outputs/iso_cond_hydro_local4_outer4000.csv
```
...
```bash
.venv/bin/python util/plot_dynamic_mix_hse_diagnostic.py --time 100 --out visualization/iso_dynamic_mix_hse_t100.png \
  outputs/iso_dynamic_mix_hse_ns2000.csv outputs/iso_dynamic_mix_hse_ns3000.csv outputs/iso_dynamic_mix_hse_ns4000.csv
```
...
```bash
.venv/bin/python util/plot_iso_frame.py outputs/iso_t22k.txt
```
...
```bash
ISO_CORONA=1 ISO_CORONA_TOP_KM=10000 ISO_C7_HSE=1 ISO_HEAT_FLUX=1 ISO_NS=1200 ./build/chromo_main outputs/iso_corona.txt full no-ionization model_isentropic - 30 no-cooling
.venv/bin/python util/animate_isentropic.py outputs/iso_corona.txt
```
...
ISO_CORONA=1 ISO_CORONA_TOP_KM=10000 ISO_H_BASE=1003 ISO_C7_HSE=1 ISO_HEAT_FLUX=1 \
    ...
    ./build/chromo_main outputs/iso_corona_full.txt full ionization model_isentropic - 10 cooling
.venv/bin/python util/animate_isentropic.py outputs/iso_corona_full.txt
```
...
```bash
.venv/bin/python util/iso_t22k_poster.py   # defaults to outputs/iso_t22k_ns2000_gamma105.txt, γ=1.05
# γ=5/3 version: ISO_GAMMA=1.6666667 .venv/bin/python util/iso_t22k_poster.py outputs/iso_t22k_ns2000.txt <out.pdf>
...
ISO_C7_IC=1 ISO_C7_HSE=1 ISO_H_BASE=0 ISO_DH=2152.6 ISO_T_TOP=22000 ISO_HEAT_FLUX=1 \
    ...
    ISO_REFINE_FACTOR=4 ISO_REFINE_S_LO_KM=0 ISO_REFINE_S_HI_KM=700 ISO_REFINE_TRANSITION_KM=100 \
    ISO_NS=2000 \
    ./build/chromo_main outputs/iso_local4_sanity.txt full no-ionization model_isentropic - 1 no-cooling
...
ISO_C7_IC=1 ISO_C7_HSE=1 ISO_H_BASE=0 ISO_DH=2152.6 ISO_T_TOP=22000 ISO_HEAT_FLUX=1 \
    ...
    ISO_DIAG_COND_HYDRO_OUT=outputs/iso_cond_hydro_local4_ns2000_main.csv \
    ./build/chromo_main outputs/iso_local4_ns2000_main.txt full no-ionization model_isentropic - 3 no-cooling

ISO_GAMMA=1.05 .venv/bin/python util/animate_isentropic.py \
    outputs/iso_local4_ns2000_main.txt visualization/iso_local4_ns2000_bestwb_evolution.mp4
```
...
ISO_C7_IC=1 ISO_C7_HSE=1 ISO_H_BASE=0 ISO_DH=2152.6 ISO_T_TOP=22000 ISO_HEAT_FLUX=1 \
    ...
    ISO_DIAG_COND_HYDRO_OUT=outputs/iso_cond_hydro_local4_ns2000_2000s.csv \
    ./build/chromo_main outputs/iso_local4_ns2000_2000s.txt full no-ionization model_isentropic - 32 no-cooling

ISO_GAMMA=1.05 .venv/bin/python util/animate_isentropic.py \
    outputs/iso_local4_ns2000_2000s.txt visualization/iso_local4_ns2000_2000s_evolution.mp4
```
