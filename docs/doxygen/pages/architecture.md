...
| Path | What it holds |
| --- | --- |
| `chromosphere.hpp` | chromosphere::Grid — the object that owns all solver state — plus `mix::` (3 release rows), `cons::`/`prim::` (7 two-fluid rows), MixtureField, MixtureFaceArrays, MixtureRhsScratch, MixtureConductionScratch, MixtureFaceFluxCapture, OuterConductionCapture. See @ref grid. |
...
| `src/eos.cpp` | Saha closure, `Gamma1` table loader, caloric inversions and equilibrium face builders. Everything in double precision, log-domain where the state is known only through logarithms; safeguarded Newton with bisection fallback. |
...
`chromosphere::Vec` is the workhorse cell-wise type and carries both the conserved state and the static mesh metric arrays. **The release storage type is `float`.**

`CHROMO_STATE_FLOAT64` (option, default `OFF`) promotes it to `double` and exists for one purpose: a controlled experiment isolating float32 representation round-off, which is a measured limitation of this solver — the per-step mass-density increment falls below half a float32 ULP of `rho` in 619 of the 661 cells of the release mesh, so the density there cannot evolve at all. **It is not a supported configuration.** It is not exercised by `scripts/release_validation.sh`, it fails 29 of the unit-test checks for reasons intrinsic to promoting the type, and no release number may be quoted from it. Build it into a separate tree so the release build is never shadowed:
