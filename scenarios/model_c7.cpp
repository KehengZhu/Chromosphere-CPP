...
Vec model_c7_ic(Grid& grid) {
    ...
    for (arma::uword k = 0; k < 2; ++k) {
        ...
        ob(cons::MOM_I) = 0.0f;
        ob(cons::MOM_N) = 0.0f;
        ob(cons::E_I)   = 1.5f * grid.k_b * n_e * (4.0f * T); // T_e doubled at outer
        ob(cons::E_N)   = 1.5f * grid.k_b * n_n * (2.0f * T);
    }

    // Inner ghost cells replicate the innermost extended cell.
    {
        ...
        grid.inner_boundary0_i(cons::E_I)   = 1.5f * grid.k_b * n_e * 2.0f * T;
        grid.inner_boundary0_i(cons::E_N)   = 1.5f * grid.k_b * n_n * T;
...
void model_c7_update_bc(Grid& grid, const Vec& xn) {
    ...
    auto refresh = [&](Vec& ob, float v_target, float u_target) {
        ...
        const float phi_g = 0.5f * (grid.phi_g_imh(grid.ns - 1) + grid.phi_g_iph(grid.ns - 1));
