...
Because `dt` and the step count are unchanged on this configuration, the CFL 0.50 acceptance sweep of `docs/coarse_model_column_physical_conduction_recap.md` — run under the Roe flux and the equilibrium signal speed — carries over to the release flux for **this configuration only**. It does not carry over to a materially different model shape.
