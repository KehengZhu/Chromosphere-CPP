...
2. float32 representation round-off in the conserved state, which `docs/studies/numerics/roe_n1000_lower_chromosphere_sloshing_diagnosis.md` had already flagged indirectly through an unexplained inconsistency between the `.faceflux` divergence and the density evolution recorded in the same file.
