...
| `docs/float32_precision_control_experiment.md` | **Current** — the authority for the storage-precision limitation | That the broad lower-chromosphere mass-flux gradient is float32 conserved-state round-off and not physics, the below-half-a-ULP mass-update mechanism, the closure of the `.faceflux` divergence anomaly, the precision-independence of the first-interior-face artifact, and the `CHROMO_STATE_FLOAT64` diagnostic build. |
| `docs/roe_n1000_lower_chromosphere_sloshing_diagnosis.md` | Current for the mode; its `.faceflux` divergence anomaly is now explained as float32 round-off; measured under the Roe reference flux | The lower-chromosphere standing mode, the reflecting inner boundary, and the mass-reservoir behaviour. |
