...
> and validation are in `docs/local_mesh_scaled_numerical_conduction_recap.md`.

Follow-up to `docs/resolution_convergence_scan_recap.md` (the `rho V` ripple in 2130–2150 km is spatial truncation error and converges at ~first order under *global* refinement) and `docs/numerical_conduction_convergence_recap.md` (`numerical_diffusivity = 2000 Δh` supplies 58 % of the outer conductive drive at `ns=1000`). This stage adds a **static local** refinement profile that puts the fine cells only where the ripple lives — the upper TR against the outer boundary — and measures the same metrics.
...
For `outer`, `s_lo` is the foot of the fully refined region, the grade sits immediately *below* it, and `s_hi` is unused. Full semantics: `docs/static_local_refinement.md`.
...
- `docs/static_local_refinement.md` — `outer` profile, the transition-as-minimum semantics, and an explicit note that refinement does *not* reduce `numerical_diffusivity` inside the fine band.
