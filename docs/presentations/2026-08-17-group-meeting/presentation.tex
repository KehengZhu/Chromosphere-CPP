...
% exact-Riemann Godunov flux, and a short note on a storage-precision fix.
%
...
\subtitle{The ionization closure, the domain, and the new face solver}
\author{Keheng Zhu}
...
\begin{frame}{Aside: the lower-chromosphere mass flux was a storage bug}
  \begin{columns}[c]
    \begin{column}{0.44\textwidth}
      ...
      it flat. Not physics, not the solver, not the boundary --- and evaporation
      at the top moves only $\sim$3\%.}
    \end{column}
...
\begin{frame}{Status and open questions}
  ...
  \begin{enumerate}\itemsep12pt
    \item \textbf{Saha vs.\ finite relaxation --- the real open question.}
          We freeze composition \emph{inside} the Riemann problem, then re-impose
          instantaneous Saha equilibrium \emph{after every step}. Not justified
          when $\tau_{\rm ion}$ is long. Finite-rate network, or a relaxation
          source?
    \item \textbf{Resolution.} N500 long-duration evaporation mass flux is not
          established as $<$10\% grid-converged.
    \item \textbf{Minor, tracked:} one localized artifact at the first interior
          face --- survives the precision control, so it is a real
          boundary-closure effect.
  \end{enumerate}
  \vfill
\end{frame}
...
\begin{frame}{Backup: sources}
  ...
  \begin{tabular}{ll}
    ...
    Figure provenance & \texttt{figures/README.md}; \texttt{visualize\_commands.md} \S14--15 \\
  ...
  \begin{center}
    ...
    Precision control: \texttt{ac3d9e0}
  \end{center}
\end{frame}
