#!/usr/bin/env python3
"""Overlay two chromo_main runs (optically-thick cooling on vs off) on the same
8-panel animation.

Usage:
    python3 animate_cooling_compare.py [cool_on.txt] [cool_off.txt] [out.mp4] [fps]
"""
import os
import sys
import numpy as np
...
def _render_frame(k, tmpdir, xx, pon_k, t_on, poff_k, t_off, nF, ylims):
    ...
    fig.suptitle(
        f"Model C7  |  optically-thick cooling ON  t = {t_on:6.2f} s"
        f"   vs   cooling OFF  t = {t_off:6.2f} s   ({k+1}/{nF})"
    )
    for row, col, idx, title, log in _PANELS:
        ...
        ax.plot(xx, pon_k[idx],  lw=1.4, color="C0", label="cooling ON (Stage R)")
        ax.plot(xx, poff_k[idx], lw=1.4, color="C3", linestyle="--", label="cooling OFF")
    for ax in axes[1, :]:
...
def main():
    ...
    on_path  = sys.argv[1] if len(sys.argv) > 1 else os.path.join(root, "outputs", "out_c7_cool_on.txt")
    off_path = sys.argv[2] if len(sys.argv) > 2 else os.path.join(root, "outputs", "out_c7_cool_off.txt")
    out_path = sys.argv[3] if len(sys.argv) > 3 else os.path.join(HERE, "..", "visualization", "model_c7_cooling_compare.mp4")
    fps      = int(sys.argv[4]) if len(sys.argv) > 4 else 6
    ...
    # Drive animation off the shorter (cooling-on) timeline, picking the
    # nearest-time off-case frame at each step. Cooling-on terminates early
    # because optically-thick radiative cooling drives a thermal collapse
    # the integrator can't follow past ~step 700.
    ...
    def lim(series_list, log=False):
        ...
        if log:
            a = a[a > 0]
        if a.size == 0:
            return (1e-30, 1.0) if log else (-1.0, 1.0)
    ...
    print(f"cooling-on : {len(t_on)} frames, t in [{t_on.min():.1f}, {t_on.max():.1f}] s")
    print(f"cooling-off: {len(t_off)} frames, t in [{t_off.min():.1f}, {t_off.max():.1f}] s")
